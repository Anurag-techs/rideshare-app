 ...
 * Now stores total_amount and payment_status
 */
router.post('/', authRequired, (req, res) => {
  try {
    const { ride_id, seats_booked, payment_intent_id } = req.body;
    ...
    // Calculate total and payment status
    const totalAmount = ride.price_per_seat * seats;
    const isFree = totalAmount <= 0;
    const paymentStatus = isFree ? 'paid' : (payment_intent_id ? 'paid' : 'unpaid');

    const bookRide = transaction(() => {
      const result = prepare(
        'INSERT INTO bookings (ride_id, passenger_id, seats_booked, total_amount, payment_status, payment_intent_id, status) VALUES (?, ?, ?, ?, ?, ?, ?)'
      ).run(ride_id, req.user.id, seats, totalAmount, paymentStatus, payment_intent_id || null, 'confirmed');
    ...
    const booking = prepare(
      "SELECT b.*, r.from_location, r.to_location, r.departure_time, r.price_per_seat, u.name as driver_name FROM bookings b JOIN rides r ON b.ride_id = r.id JOIN users u ON r.driver_id = u.id WHERE b.id = ?"
    ).get(bookingId);
